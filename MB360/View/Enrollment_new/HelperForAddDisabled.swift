...
extension AddDisabledTwinsVC : UIGestureRecognizerDelegate,UITextFieldDelegate {
...
extension AddDisabledTwinsVC {
     func setDateToolBar()
    {
        ...
        if isEdit == true {
            ...
            self.datePicker1.setDate(oldDate, animated: false)
            self.datePicker2.setDate(datePicker1.date, animated: false)
        }
        self.datePicker2.setDate(datePicker1.date, animated: false)
       
//            let dateOld = Calendar.current.date(byAdding: .year, value: -25, to: Date())
...
//
        ...
        //if isChange1{
            if isDiffAbled1 == false {
                datePicker1.minimumDate = datemin
            }else if isDiffAbled1 == true{
                datePicker1.minimumDate = datemin1
            }
//        }else{
//            datePicker1.minimumDate = datemin
//        }
        ...
        //if isChange2 {
            if isDiffAbled2 == false {
                datePicker2.minimumDate = datemin
            }else if isDiffAbled2 == true{
                datePicker2.minimumDate = datemin1
            }
//        }else{
//                datePicker2.minimumDate = datemin
//        }
        
        self.txtDob.inputView = datePicker1
        self.txtDob.addTarget(self, action: #selector(myTargetFunction), for: .touchDown)

        self.txtDob2.inputView = datePicker2
    ...
    @objc func doneClick() {
        ...
        //txtDob2.text = formatter.string(from: datePicker1.date)
        
        let dateString = dateFormatter1.string(from: datePicker1.date)
        print(dateString)
        
        let serverDt = Date()
        self.firstChildAge = serverDt.years(from: datePicker1.date)
...
//            isDiffAbled1 = true
    ...
    @objc func doneClickSecondChild() {
           ...
           txtDob2.text = formatter.string(from: datePicker2.date)

           let dateString = dateFormatter1.string(from: datePicker2.date)
           print(dateString)
           
           let serverDt = Date()
           self.secondChildAge = serverDt.years(from: datePicker2.date)
